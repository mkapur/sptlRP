...
for(v in 1:length(Ftest)){
  ...
  for(k in 1:2){ ## Loop over steps A & B
    ...
    for(i in 1:narea){ ## will overwrite second time
      ...
      Yield_Ri[v,i] <- prop$Yield_i[i]/(rec_level*rdistUse[i])
    ...
    # if(v > 100 ) cat(Ftest[v]," ",recr_dist_adj, "\n"); next()
...
plot(proposed_i[,,3], ylim = c(0,100))
points(proposed_i[,,2])
points(proposed_i[,,1])
  
plot(proposed_i[,c(1,3),3], ylim = c(0,500), col = 'red')
