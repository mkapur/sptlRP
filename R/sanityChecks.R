...
png(here('figs','testdoPR-movement.png'))
par(mfrow = c(2,2))
...
plot(tmp$NPR[1,,1], main = 'A1 SRC - defaults');points(tmp$NPR[2,,1],col = 'blue') ## should look reasonable
legend('topright', pch =1, legend = c('stay in a1', 'end up in a2'), col = c('black','blue'))
plot(tmp$NPR[1,,2], main = 'A2 SRC - defaults', ylim = c(0,1));points(tmp$NPR[2,,2],col = 'blue') ## should look reasonable
