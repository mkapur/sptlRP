...
for(RR in 1:length(R0_list)){
  for(Fv in 1:length(Ftest)){
    curr <- run_one_current(Fv_i = c(Ftest[Fv]*0.8,Ftest[Fv]*0.1,Ftest[Fv]*0.1 ),#rep(Ftest[Fv],narea), 
                            rec_level_idx = RR, 
                            recr_dist= recr_dist, 
                            movemat = X_ija)
    prop <- optim_loop2(Fv_i = c(Ftest[Fv]*0.8,Ftest[Fv]*0.1,Ftest[Fv]*0.1 ), #rep(Ftest[Fv],narea), 
                       rec_level_idx = RR, 
                       # receq= receq,
                       recr_dist= recr_dist, 
                       movemat = X_ija)
...
ggsave( plot_yield_curves(sys_matrix  = sys3area, byarea = FALSE),
        file = here('figs',paste0('Yield_comparisons_3area_buff0.005_5ppenalty_',Sys.Date(),'.png')),
       width = 10, height = 8, unit = 'in', dpi = 520)


ggsave( plot_yield_curves(sys3area = sys3area,rRef_proposed_i, byarea = TRUE),
...
for(RR in 1:length(R0_list)){
  ...
  for(Fv in 1:length(Ftest)){
    curr <- run_one_current(Fv_i = rep(Ftest[Fv],narea),  # c(Ftest[Fv]*1,Ftest[Fv]*1)
                            rec_level_idx = RR, 
                            recr_dist= recr_dist, 
                            movemat = X_ija_MIX2)
    # cat(curr$Yield,"\n")
...
#         file = here('figs',paste0('Yield_comparisons_2area_buff0.005_5ppenalty_',Sys.Date(),'.png')),
#         width = 10, height = 8, unit = 'in', dpi = 520)
...
#                        Sys.Date(),'.png')),
#     height = 8.5, width = 11, unit = 'in', res = 600)
plot_radj(radj_kvar = kvar_radj_2area)
# dev.off()
